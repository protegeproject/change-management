...
public class OntologyComponentFactory {
    ...
    public Cls getOntologyClass() {
        final String name = "Ontology";
        return kb.getCls(name);
    }

    public Ontology createOntology(String name) {
        Cls cls = getOntologyClass();
        Instance inst = cls.createDirectInstance(name);
        return new DefaultOntology(inst);
    }

    public Ontology getOntology(String name) {
        return OntologyJavaMappingUtil.getSpecificObject(kb, kb.getInstance(name), Ontology.class);
    }

    public Collection<Ontology> getAllOntologyObjects() {
        return getAllOntologyObjects(false);
    }

    public Collection<Ontology> getAllOntologyObjects(boolean transitive) {
        Collection<Ontology> result = new ArrayList<Ontology>();
        final Cls cls = getOntologyClass();
        for (Object element : transitive ? cls.getInstances() : cls.getDirectInstances()) {
            Instance inst = (Instance) element;
            result.add(OntologyJavaMappingUtil.getSpecificObject(kb, inst, Ontology.class));
        }
        return result;
    }
