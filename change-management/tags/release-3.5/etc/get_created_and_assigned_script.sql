...
SELECT icd_sl.short_value AS sorting_label, icd_title_label.short_value AS title,
       ...
       oc.short_value AS created_class_id, ts.short_value AS timestamp
  ...
  LEFT JOIN icd_ann AS change_timestamp ON change_timestamp.slot='timestamp' AND all_changes.frame=change_timestamp.frame
  JOIN icd_ann AS ts ON ts.slot='date' AND change_timestamp.short_value=ts.frame
